...
int main()
{
    ...
    Shader bumpMappingShader("../shaders/vertex/normal_mapping.vs", "../shaders/fragment/normal_mapping.fs");
    ...
    VAO LightBoxVAO, planeVAO, cubeMapVAO;
    ...
    bumpMappingShader.setInt("diffuseMap", 0);
    bumpMappingShader.setInt("normalMap", 1);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec3 cloudPosition = glm::vec3(0.0f, 2.0f, -10.0f);
        ...
        bumpMappingShader.Activate();
        bumpMappingShader.setMat4("projection", projectionRender);
        bumpMappingShader.setMat4("view", viewRender);
        glm::mat4 modelRender = glm::mat4(1.0f);
        modelRender = glm::translate(modelRender, glm::vec3(-0.0f, -0.0f, -3.0f));
        bumpMappingShader.setMat4("model", modelRender);
        bumpMappingShader.setVec3("viewPos", camera.Position);
        bumpMappingShader.setVec3("lightPos", lightPos);
        glActiveTexture(GL_TEXTURE0);
        ...
        glm::vec3 StarPosition_five = glm::vec3(0.0f, 3.0f, 3.0f);
        ...
        glm::vec3 StarPosition_7 = glm::vec3(5.0f, 2.0f, 6.0f);
        ...
        glm::vec3 StarPosition_11 = glm::vec3(-3.0f, 1.0f, 1.0f);
        ...
        // RAIN
        glm::vec3 rainPosition = cloudPosition + glm::vec3(-1.0f, -2.0f, -1.0f);
        glActiveTexture(GL_TEXTURE0);
        textureWater.Bind();
        glActiveTexture(GL_TEXTURE1);
        textureWater.Bind();
...
void renderScene(Shader &shader, Model &backpackModel, VAO &planeVAO, Model &beachBallModel, glm::vec3 newBallPos, Model &showerModel, Model &sunModel, glm::vec3 lightPos, Model &cloudModel, glm::vec3 cloudPosition)
{
    ...
    if (lightPos.y > 4.0f)
    {
        shader.setFloatReal("ambientStrength", 0.5f);
    }
    else if (lightPos.y > 0.2f && lightPos.y < 4.0f)
    {
        shader.setFloatReal("ambientStrength", (0.15 * lightPos.y) - 0.1);
    }
    else
    {
